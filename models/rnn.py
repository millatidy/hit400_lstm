...
class RNN:
    ...
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.input_to_hidden_weights = np.random.uniform(
            -np.sqrt(1. / input_dim), np.sqrt(1. / input_dim), (hidden_dim, input_dim))
        self.hidden_to_hidden_weights = np.random.uniform(
            -np.sqrt(1. / hidden_dim), np.sqrt(1. / hidden_dim), (hidden_dim, hidden_dim))
        self.hidden_to_output_weights = np.random.uniform(
    ...
    def forward_propagation(self, input_array):
        ...
        for t in np.arange(N):
            ...
            hidden_state[t] = np.tanh(self.input_to_hidden_weights[:, input_array[
                                      t]] + self.hidden_to_hidden_weights.dot(hidden_state[t-1]))
            output[t] = self.softmax(
                self.hidden_to_output_weights.dot(hidden_state[t]))
